...
import com.routegenius.backend.service.MailService; // <<< ADDED THIS IMPORT
...
public class NotificationServiceImpl implements NotificationService {
    ...
    private final MailService mailService; // <<< INJECTED MailService
    ...
    public void generateNotificationForParcelStatusChange(Long parcelId, ParcelStatus newStatus) {
        ...
        String prompt = String.format(
                "Generate a concise, customer-friendly notification message for a parcel status update. " +
                        "The parcel has Tracking ID: %s. " +
                        "Sender: %s (%s, %s). Recipient: %s (%s, %s). " +
                        "Current status: %s. Current location: %s, %s, %s. " +
                        "Estimated delivery date: %s. " +
                        "Focus on the new status: %s. " +
                        "Example: 'Your parcel (Tracking ID: ABC123) is now in transit. It departed from New York and is heading towards Los Angeles. Estimated delivery: 2025-08-01.'",
                parcel.getTrackingId(),
                sender.getFirstName(), sender.getEmail(), parcel.getSenderAddress(),
                recipient.getFirstName(), recipient.getEmail(), parcel.getRecipientAddress(),
                parcel.getStatus().name().replace("_", " "), parcel.getCurrentLocation(), parcel.getCurrentCity(), parcel.getCurrentCountry(),
                parcel.getEstimatedDeliveryDate() != null ? parcel.getEstimatedDeliveryDate().toLocalDate().toString() : "N/A",
    ...
    private String callGeminiApi(String prompt) throws Exception {
        ...
        // This is the URL structure for the Gemini API
